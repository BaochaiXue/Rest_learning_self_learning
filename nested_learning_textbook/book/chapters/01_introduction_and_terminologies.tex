...
本书涉及多个研究社区的术语，它们有时指代相同的概念。
以下对照表帮助读者在不同章节之间保持术语一致。

\begin{center}
\begin{tabular}{p{3.5cm} p{3.5cm} p{5cm}}
\toprule
\textbf{术语} & \textbf{同义词 / 近义词} & \textbf{本书中的含义} \\
\midrule
预训练（Pretraining）
& 外循环训练
& 在大规模数据集上学习 $\theta$ \\
...
本书的核心论点是：``学习''发生在多个时间尺度上。
以下表格总结了不同时间尺度对应的变量和章节。

\begin{center}
\begin{tabular}{p{2.5cm} p{2cm} p{3.5cm} p{4cm}}
\toprule
\textbf{时间尺度} & \textbf{符号} & \textbf{更新频率} & \textbf{首次出现} \\
\midrule
最慢：预训练
& $\theta$
& 整个数据集（数周）
...
%─────────────────────────────────────────────
\section{后续章节中反复出现的主题}
\label{sec:intro_themes}
%─────────────────────────────────────────────

以下概念将在全书11章中反复出现。
在这里先建立预期，后续遇到时可以回溯本节。

\begin{enumerate}
  \item \textbf{外积更新}（$vk^\top$）：
    第~\ref{ch:fastweights}~章引入，
    在第~\ref{ch:attention}、\ref{ch:icl}、\ref{ch:unified}~章反复出现。
    是联想记忆、梯度步、快权重更新的统一数学形式。

  \item \textbf{双层优化}（内循环 + 外循环）：
    第~\ref{ch:bilevel}~章引入，
    在第~\ref{ch:ttt_bridge}、\ref{ch:nested}~章推广为多层嵌套。

  \item \textbf{``查询-键-值''三元组}（$q, k, v$）：
    第~\ref{ch:attention}~章引入，
    贯穿所有后续章节。

  \item \textbf{``隐状态是参数''的观点}：
    第~\ref{ch:ttt_bridge}~章首次出现，
    在第~\ref{ch:ttt_layers}、\ref{ch:titans}、\ref{ch:nested}~章中
    成为核心架构设计原则。

  \item \textbf{``机制等价 vs.\ 解释性视角''的区分}：
    第~\ref{ch:icl}~章引入，
    在第~\ref{ch:unified}、\ref{ch:nested}~章成为学术诚信的核心议题。
\end{enumerate}
